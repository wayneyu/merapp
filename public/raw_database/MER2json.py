...
def write_latex(file_list, title, where_to_save, overwrite=False):
    df = pd.read_csv('questions_meta.csv', header=None,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-latex', dest='latex',
                        action='store_false',
                        help='only download, do not compile latex files')
    ...
    title = ''
    if not args.topic == '/':
        df = pd.read_csv('questions_topic.csv')
    ...
    if '/' == args.topic and '/' == args.course and '/' == args.exam:
        title = 'All questions from all exams'
